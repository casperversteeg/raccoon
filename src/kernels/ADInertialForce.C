...
registerMooseObject("raccoonApp", ADInertialForce);

InputParameters
ADInertialForce::validParams()
{
...
ADInertialForce::ADInertialForce(const InputParameters & parameters)
...
Real
ADInertialForce::computeQpResidual()
{
  if (_dt == 0)
    return 0;
  else if (_has_beta)
  {
    ...
    return _test[_i][_qp] * MetaPhysicL::raw_value(_density[_qp]) *
           (accel + vel * MetaPhysicL::raw_value(_eta[_qp]) * (1 + _alpha) -
            _alpha * MetaPhysicL::raw_value(_eta[_qp]) * (*_vel_old)[_qp]);
  }
  ...
  // Same for explicit, implicit, and implicit with HHT
    ...
    return _test[_i][_qp] * MetaPhysicL::raw_value(_density[_qp]) *
           ((*_u_dotdot_factor)[_qp] +
            (*_u_dot_factor)[_qp] * MetaPhysicL::raw_value(_eta[_qp]) * (1.0 + _alpha) -
            _alpha * MetaPhysicL::raw_value(_eta[_qp]) * (*_u_dot_old)[_qp]);
}

void
ADInertialForce::computeResidualAdditional()
{
  if (_dt == 0)
    return;
  ...
  for (unsigned int i = 0; i < _test.size(); ++i)
    _local_re(i) *=
        (*_u_dotdot_factor_dof)[i] + MetaPhysicL::raw_value(_eta[0]) * (*_u_dot_factor_dof)[i];
}

Real
ADInertialForce::computeQpJacobian()
{
  if (_dt == 0)
    return 0;
  else
  {
    if (_has_beta)
      return _test[_i][_qp] * MetaPhysicL::raw_value(_density[_qp]) / (_beta * _dt * _dt) *
                 _phi[_j][_qp] +
             MetaPhysicL::raw_value(_eta[_qp]) * (1 + _alpha) * _test[_i][_qp] *
                 MetaPhysicL::raw_value(_density[_qp]) * _gamma / _beta / _dt * _phi[_j][_qp];
    else
      return _test[_i][_qp] * MetaPhysicL::raw_value(_density[_qp]) * (*_du_dotdot_du)[_qp] *
                 _phi[_j][_qp] +
             MetaPhysicL::raw_value(_eta[_qp]) * (1 + _alpha) * _test[_i][_qp] *
                 MetaPhysicL::raw_value(_density[_qp]) * (*_du_dot_du)[_qp] * _phi[_j][_qp];
  }
}
